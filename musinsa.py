...
for item in items:
    #아래코드를 실행해봐!
    try:
        #개별 상품에 대한 정보를 가져오기get
        # find_element(CSS를 기준으로 찾아줘, a['이름']): '이름'을 갖고있는 블럭
        # find_element ->작은범위에서 데이터를 찾아오겠다. /
        a_tag = item.find_element(By.CSS_SELECTOR, "a[data-original-price]")

        #상품이름
        name = a_tag.get_attribute("aria-label")
        # 예외처리
        # str(name) ->name에 뭐가 들어오든 str처리하겠다.
        # replace -> "제거할 데이터",""
        # strip -> 양쪽 공백 제거
        ...
        brand = str(brand).strip() if brand else "브랜드없음"

        #가격
        price =a_tag.get_attribute("data-price")
        price = str(price).strip() if price else "가격없음"

        #상세페이지
        # href (html에서 '하이퍼링크'를 의미, 이미지/미디어로 연결되는 태그)
